...
|----|------|--------|--------|
| TD-001 | SKILL.md 的工作流步骤与实际 CLI 命令不完全对应 | open | Stage 1 |
| TD-002 | `data/dashboard_data.json` 实际不存在，完全依赖浏览器 localStorage | open | Stage 2 |

### High
| ID | Item | Status | Target |
|----|------|--------|--------|
| TD-003 | 无自动化测试，手动验证效率低 | resolved | Stage 2 |
| TD-004 | 硬编码路径 `E:/OpenCode/bioinfo-learning-skill/` 出现在多份文档中 | open | Stage 1 |
| TD-005 | 无 CI 配置，无法自动化校验 PR | resolved | Stage 1 |

### Medium
| ID | Item | Status | Target |
|----|------|--------|--------|
| TD-006 | `scripts/` 与 `.claude/skills/bioinfo-learning/scripts/` 两个脚本目录职责不清 | open | Stage 1 |
| TD-007 | 无日志系统，操作不可追溯 | resolved | Stage 2 |
| TD-008 | Docker 环境脚本（install_bioinfo_tools.sh/ps1）未经测试 | open | Stage 3 |

### Low
| ID | Item | Status | Target |
|----|------|--------|--------|
| TD-009 | 4C 优化数据（weeks_4c/）与主 curriculum.json 未合并 | open | Stage 3 |
| TD-010 | README.md 中的示例命令路径与 Linux/macOS 不完全兼容 | open | Stage 1 |
| TD-011 | `.gitignore` 缺少 `logs/`、`reports/`、`__pycache__/` 模式 | open | Stage 1 |

### DevOps
| ID | Item | Status | Target |
|----|------|--------|--------|
| TD-012 | 无健康度自动报告 | resolved | Stage 3 |
...
- [x] `docs/data-schema.md` — 数据格式规范
- [x] `docs/commands.md` — 命令参考
- [x] `docs/development.md` — 贡献指南
- [x] `.claude/lint/forbidden-patterns.json` — 禁止模式规则
- [x] `.claude/lint/structure-rules.json` — 结构规则
- [x] `.github/workflows/skill-validate.yml` — Skill 校验 CI
- [x] `.github/workflows/data-validate.yml` — 数据校验 CI
- [x] `.claude/scripts/detect-harness.py` — 检测脚本

### 阶段二（可观测闭环 — 已完成）
- [x] `tests/test_skill_structure.py` — Skill 结构测试
- [x] `tests/test_bioinfo_cli.py` — CLI 测试
- [x] `tests/test_data_integrity.py` — 数据完整性测试
- [x] `tests/__init__.py`
- [x] `logs/harness-session.json` — 会话记录
- [x] `.claude/scripts/auto-fix-data.py` — 数据自动修复

### 阶段三（熵治理 — 已完成）
- [x] `.claude/scripts/health-report.py` — 健康度报告生成
- [x] `reports/health/` — 报告输出目录
- [x] 技术债清单（本节）
